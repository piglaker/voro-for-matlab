...
void help_message() {
	mexPrintf("Voro++ version 0.4.5, by Chris H. Rycroft (UC Berkeley/LBL)\n\n"
...
void custom_output_message() {
	mexPrintf("The \"-c\" option allows a string to be specified that will customize the output\n"
...
void version_message() {
	mexPrintf("Voro++ version 0.4.5 (July 27th 2012)");
}

// Prints an error message. This is called when the program is unable to make
// sense of the command-line options.
void error_message() {
	mexPrintf("voro++: Unrecognized command - line options; type \"voro++ -h\" for more\ninformation.\n" );
...
int voro_main(int argc, char **argv) {
	...
	bool xperiodic = false, yperiodic = false, zperiodic = false, ordered = false, verbose = true;
	...
	if (verbose) {
		mexPrintf("Container geometry        : [%g:%g] [%g:%g] [%g:%g]\n"
		...
		mexPrintf("Total imported particles  : %d (%.2g per grid block)\n"
